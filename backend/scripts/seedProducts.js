/**
 * Seed ~10 demo products per category with category-matching Unsplash images.
 *
...
// Verified working Unsplash URLs grouped by category
...
async function seed() {
		...
		items.forEach(([name, description, price, isFeatured], index) => {
			...
			const styles =
				category === "jeans"
					...
					: category === "t-shirts"
						? ["Crew Neck", "V-Neck", "Oversized"]
						: category === "shoes"
							? ["Casual", "Sport", "Formal"]
							: category === "glasses"
								? ["Aviator", "Wayfarer", "Round"]
								: category === "jackets"
			...
			const perVariantStock = 3 + ((index * 2) % 8); // 3–10 each size×color
			...
			// Make first combo intentionally zero sometimes for demo OOS
			if (variants[0] && index % 4 === 0) variants[0].stock = 0;
	...
	console.log(`Inserted ${inserted.length} products with category-matched images`);

	try {
		const featuredProducts = await Product.find({ isFeatured: true }).lean();
		await redis.set("featured_products", JSON.stringify(featuredProducts));
		console.log(`Updated featured cache (${featuredProducts.length} items)`);
	} catch (error) {
		console.log("Featured cache update skipped:", error.message);
	}
