...
export const isEmailConfigured = () =>
  isHttpsEmailConfigured() || isSmtpConfigured();
...
const isUnverifiedFrom = (value = "") =>
  /example\.com|ecommerce\.local|gmail\.com|yahoo\.com|outlook\.com/i.test(
    value,
  );
...
const createSmtpTransport = (port) => {
  ...
  const isGmail =
    /gmail\.com/i.test(SMTP_HOST) || /gmail\.com/i.test(SMTP_USER);
...
const hydrateOrderProducts = async (orderProducts = []) => {
  ...
  const docs = await Product.find({ _id: { $in: ids } }).select(
    "name images image",
  );
...
export const sendEmail = async ({
  ...
  const trySend = async (payload) => {
    const isGmail =
      /gmail\.com/i.test(envVal("SMTP_HOST")) ||
      /gmail\.com/i.test(envVal("SMTP_USER"));
    const ports = isGmail
      ? [587, 465]
...
export const sendOrderEmail = async (order, event) => {
  try {
    ...
    let customerName =
      typeof order.user === "object" ? order.user.name : "there";
    if (!email) {
    ...
    if (!email) {
      console.warn(
        "[email] skip order mail — no customer email on order",
        event,
      );
      return;
    }
...
Thank you for shopping with us.`;
    ...
    if (!result.ok) {
      console.error("[email] order mail not delivered", {
        to: email,
        event,
        error: result.error,
      });
